<div class="max-w-5xl mx-auto py-10 px-4">
    ...
    <header class="mb-10 flex flex-col md:flex-row justify-between items-center gap-6">
        <div class="flex items-center gap-4">
            <a href="universe.php?id=<?= (int)$list['profile_id'] ?>" class="p-2 bg-white border border-slate-200 text-slate-400 rounded-xl hover:text-<?= $color ?>-600 hover:border-<?= $color ?>-200 transition-all shadow-sm group" title="Retour à l'Univers">
        ...
        <div class="flex gap-3">
            ...
            <a href="index.php?list=<?= (int)$list['id'] ?>" class="px-4 py-2.5 rounded-xl font-bold text-white bg-slate-900 hover:bg-slate-800 transition-all flex items-center gap-2 shadow-sm text-sm">
        ...
        <div class="flex items-center gap-4">
            ...
            <div class="flex gap-2">
                ...
                <?php foreach($categories as $cat): ?>
                    <a href="?id=<?= (int)$list['id'] ?>&cat=<?= urlencode($cat) ?>" 
                ...
                <div data-id="<?= $item['id'] ?>" class="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 flex flex-col group relative transition-all hover:shadow-md <?= $item['priority'] == 3 ? "ring-1 ring-amber-400" : '' ?>">
                    ...
                    <div class="flex gap-4 mt-auto pt-3 border-t border-slate-50">
                        ...
                        <?php if (!empty($item['url'])): ?>
                            <a href="<?= htmlspecialchars($item['url']) ?>" target="_blank" rel="noreferrer noopener" class="text-[9px] font-bold text-slate-400 hover:text-<?= $color ?>-600 uppercase tracking-widest">Voir</a>
                        <?php endif; ?>
